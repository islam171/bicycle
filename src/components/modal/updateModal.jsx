...
const UpdateModal = React.memo(({closeModal, data, func, token}) => {
    ...
    const onSubmit = (values) => {
        func === "category" && dispatch(updateCategory({_id:data._id, token, values}))
        func === "material" && dispatch(updateMaterial({_id:data._id, token, values}))
        func === "color" && dispatch(updateColor({_id:data._id, token, values}))
        func === "package" && dispatch(updatePackage({_id:data._id, token, values}))
        closeModal()
    }
    ...
    return (
        <div className={"p-10 bg-white shadow-amber-400 relative"}>
            ...
            <div className={"my-5 text-2xl font-medium "}>Обновление
                {
