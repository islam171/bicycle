...
const SideBarMenu = () => {
	...
	useClickOutside(SideBarMenuRef, () => {
		dispatch(closeSidebarMenu())
	})

	return (
		<div ref={SideBarMenuRef} onClick={e => e.stopPropagation()}>
			<div
			...
			>
				...
				<div className={'px-3'}>
					<div>
						<h1 className={'py-5 border-b-2 font-bold'}>
							<Link to={'katalog'}>Каталог</Link>
						</h1>
						<MenuBar title={'Материал'}>
							{materials.length > 0 &&
								materials.map(material => (
									<Link to={'katalog'} key={material._id}>
										{material.name}
									</Link>
								))}
						</MenuBar>
							...
							{colors.length > 0 &&
								colors.map(colors => (
									<Link
										to={'katalog'}
										key={colors._id}
										// onClick={() => dispatch(setColor(colors._id))}
									>
										{colors.name}
									</Link>
								))}
