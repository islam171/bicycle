...
import BicycleMaking from '../entities/Bicycle/ui/BicycleMaking'
...
const Making = () => {
	...
	const onNewOrder = values => {
		for (const item of makingCart) {
			values.bicycles = values.bicycles
				? [
						...values.bicycles,
						{ _id: item.cart.bicycle._id, count: item.count }
					]
				: [{ _id: item.cart.bicycle._id, count: item.count }]
		}
						...
						<MenuBar>
							<div className={'px-2'}>
								<div className={'flex flex-col'}>
									{makingCart.map(item => (
										<BicycleMaking key={item.cart._id} item={item} />
									))}
								</div>
					...
					<div className={'mt-10'}>
						<form action='' onSubmit={handleSubmit(onNewOrder)}>
							...
							<textarea
								...
								{...register('comment')}
								className={
									'border-2 border-gray-800 transition delay-50 w-full mb-5 p-2 focus:outline-none focus:border-slate-300 focus:shadow-slate-300'
								}
							/>
							<br />
