...
import { removeCart } from '../../../entities/Cart'
...
const AdminBicycleItem = React.memo(({ bike, dispatch, token }) => {
	return (
		<>
			<div
				className={
					'relative transition delay-500 p-2 flex gap-5 max-sm:flex-col max-sm:items-center max-sm:w-60'
				}
			>
				<div
					className={
						'w-32 h-20 min-w-32 flex items-center justify-center bg-gray-100'
					}
				>
					<img
				...
				<div
					className={
						'flex flex-col max-sm:flex-row max-sm:justify-between max-sm:w-full'
					}
				>
					<div>
						<AiOutlineDelete
