...
import {
	closeSidebarFilterAdmin,
	closeSidebarMenu
} from '../../store/sidebarSlice'
import MenuBar from '../../features/MenuBar/ui/MenuBar'
import CustomInput from '../../components/Inputs/CustomInput'
import CheckIcon from '@mui/icons-material/Check'
import SidebarHeaderMobile from '../../widgets/Sidebars/ui/SidebarHeaderMobile'
...
const AdminSideBarFilter = () => {
	...
	//Filter
	const { category, color, material, sort, order, price, search } = useSelector(
		state => state.admin
	)
	...
	const SideBarAdminRef = useRef(null)

	useClickOutside(SideBarAdminRef, () => {
		dispatch(closeSidebarFilterAdmin())
	})
		...
		<div ref={SideBarAdminRef}>
			...
			>
				...
				<div className={'px-3 pb-5 h-full'}>
					<div className={'flex flex-col h-full'}>
						<div className={'border-b-2 flex justify-between'}>
							...
							<button className={''} onClick={() => dispatch(clearState())}>
								Сброс
							</button>
						</div>
								...
								<div className={'flex flex-col gap-5'}>
									...
									{categories &&
										categories.map(cat => (
											<div
												key={cat._id}
												onClick={() => dispatch(setCategory(cat._id))}
												className={'flex text-base items-center gap-2'}
											>
												<span className={'w-6 h-6 bg-gray-300 relative'}>
													{cat._id === category ? (
								...
								<div className={'flex flex-col gap-5'}>
									...
									{colors &&
										colors.map(col => (
											<div
												key={col._id}
												onClick={() => dispatch(setColor(col._id))}
												className={'flex text-base items-center gap-2'}
											>
												<span className={'w-6 h-6 bg-gray-300 relative'}>
													{col._id === color ? (
								...
								<div className={'flex flex-col gap-5'}>
									...
									{materials &&
										materials.map(mat => (
											<div
												key={mat._id}
												onClick={() => dispatch(setMaterial(mat._id))}
												className={'flex text-base items-center gap-2'}
											>
												<span className={'w-6 h-6 bg-gray-300 relative'}>
													{mat._id === material ? (
				...
				<div
					...
					style={{ left: 400 }}
					onClick={() => dispatch(closeSidebarFilterAdmin())}
				>
				...
				<div
					className={
						'bg-gray-900 w-full h-16 text-white text-center flex justify-center items-center font-bold text-lg'
					}
					onClick={() =>
						onFilterSubmit(
							search,
